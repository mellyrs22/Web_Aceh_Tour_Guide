...
$query = mysql_query("select * from pick_up where id_pick_up='$id'") or die(mysql_error());
...
<head>
	...
	<title>Reservation| TourAceh</title>	
...
<li id="menu-item-121" class="menu-item menu-item-type-post_type menu-item-object-page current-menu-item page_item page-item-115 current_page_item menu-item-121"><a href="report2.php">Report</a></li>
</ul></div>				</nav>				
...
<font color="GREEN" face="Kristen ITC" size="4" > Reserv Now !</font>
</marquee></strong><br><br><br>
<form action="../Admin/edit_pick_up_action.php" method="POST">	
		<table border = 0 width="472" height="203">
	  <input type="hidden" id="full-name" name="id_pick_up" value= "<?php echo $row[0] ?>" />
  <tr>
    <td width="166" class="nowrap"><strong>No Reserv</strong></td>
    <th width="161" ><div align="left"><strong>
      <input type="text" id="no_reserv" name="no_reserv" value="<?php echo $row[1] ?>" placeholder="No Reserv" />
    </strong></div></th>
  </tr>
  <tr>
    <td class="nowrap"><strong>Date</strong></td>
    <th width="161" ><div align="left"><strong>
      <input type="date" id="date" name="date" value="<?php echo $row[2] ?>"  placeholder=" Date"/>
    </strong></div></th>
  </tr>
  <tr>
    <td class="nowrap"><strong>No_Flight</strong></td>
    <th width="161" ><div align="left"><strong>
      <input type="text" id="no_flight"  name="no_flight" value="<?php echo $row[3] ?>" placeholder="No Flight"  />
    </strong></div></th>
  </tr>
  <tr>
    <td class="nowrap" ><strong>Address</strong></td>
    <th width="161"><div align="left"><strong>
      <textarea name="address" id="address" cols=40 rows=5 placeholder="Address"><?php echo $row[4] ?></textarea>
    </strong></div></th>
  </tr>
  <tr>
    <td class="nowrap" ><strong>Status</strong></td>
    <th width="161"><div align="left"><strong>
	<?php if ($row['5'] === "Ready") : ?>
            <input type="radio" name="status" value="Ready" id="Ready" checked /><label for="Ready">Ready</label>
            <input type="radio" name="status" value="Not Yet" id="Ready" /><label for="Not Yet">Not Yet</label>
            <?php else : ?>
            <input type="radio" name="status" value="Ready" id="Ready" /><label for="Ready">Ready</label>
            <input type="radio" name="status" value="Not Yet" id="Not Yet" checked /><label for="Not Yet">Not Yet</label>
            <?php endif; ?>
